"""
Connecticut State Enrichment Script

Discovers meeting minutes sources for all Connecticut municipalities using
domain verification and URL pattern probing.

Run:
    DATABASE_URL=... python3 scripts/enrich_connecticut.py
"""
import os
import sys
...
def main():
    print('='*60)
    print('CONNECTICUT ENRICHMENT')
    print('='*60)

    enricher = EnrichmentEngine(request_delay=1.0, timeout=10)
    results = enricher.enrich_state('CT', progress_callback=progress_callback)
    ...
    print(f'\n✓ Connecticut enrichment complete!')
