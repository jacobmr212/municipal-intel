"""
District of Columbia Enrichment Script

Discovers meeting minutes sources for District of Columbia using
domain verification and URL pattern probing.

Run:
    DATABASE_URL=... python3 scripts/enrich_dc.py
"""
import os
import sys
...
def main():
    print('='*60)
    print('DISTRICT OF COLUMBIA ENRICHMENT')
    print('='*60)

    enricher = EnrichmentEngine(request_delay=1.0, timeout=10)
    results = enricher.enrich_state('DC', progress_callback=progress_callback)
    ...
    print(f'\n✓ District of Columbia enrichment complete!')
