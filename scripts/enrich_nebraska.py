"""
Nebraska State Enrichment Script

Discovers meeting minutes sources for all Nebraska municipalities using
domain verification and URL pattern probing.

Run:
    DATABASE_URL=... python3 scripts/enrich_nebraska.py
"""
import os
import sys
...
def main():
    print('='*60)
    print('NEBRASKA ENRICHMENT')
    print('='*60)

    enricher = EnrichmentEngine(request_delay=1.0, timeout=10)
    results = enricher.enrich_state('NE', progress_callback=progress_callback)
