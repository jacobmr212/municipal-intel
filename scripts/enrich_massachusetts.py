"""
Massachusetts State Enrichment Script

Discovers meeting minutes sources for all Massachusetts municipalities using
domain verification and URL pattern probing.

Run:
    DATABASE_URL=... python3 scripts/enrich_massachusetts.py
"""
import os
import sys
...
def main():
    print('='*60)
    print('MASSACHUSETTS ENRICHMENT')
    print('='*60)

    enricher = EnrichmentEngine(request_delay=1.0, timeout=10)
    results = enricher.enrich_state('MA', progress_callback=progress_callback)
    ...
    print(f'\n✓ Massachusetts enrichment complete!')
