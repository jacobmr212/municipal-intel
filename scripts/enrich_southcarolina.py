"""
South Carolina State Enrichment Script

Discovers meeting minutes sources for all South Carolina municipalities using
domain verification and URL pattern probing.

Run:
    DATABASE_URL=... python3 scripts/enrich_southcarolina.py
"""
import os
import sys
...
def main():
    print('='*60)
    print('SOUTH CAROLINA ENRICHMENT')
    print('='*60)

    enricher = EnrichmentEngine(request_delay=1.0, timeout=10)
    results = enricher.enrich_state('SC', progress_callback=progress_callback)
    ...
    print(f'\n✓ South Carolina enrichment complete!')
