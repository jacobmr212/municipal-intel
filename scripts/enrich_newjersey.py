"""
New Jersey State Enrichment Script

Discovers meeting minutes sources for all New Jersey municipalities using
domain verification and URL pattern probing.

Run:
    DATABASE_URL=... python3 scripts/enrich_newjersey.py
"""
import os
import sys
...
def main():
    print('='*60)
    print('NEW JERSEY ENRICHMENT')
    print('='*60)

    enricher = EnrichmentEngine(request_delay=1.0, timeout=10)
    results = enricher.enrich_state('NJ', progress_callback=progress_callback)
    ...
    print(f'\n✓ New Jersey enrichment complete!')
