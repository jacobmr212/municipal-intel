"""
West Virginia State Enrichment Script

Discovers meeting minutes sources for all West Virginia municipalities using
domain verification and URL pattern probing.

Run:
    DATABASE_URL=... python3 scripts/enrich_westvirginia.py
"""
import os
import sys
...
def main():
    print('='*60)
    print('WEST VIRGINIA ENRICHMENT')
    print('='*60)

    enricher = EnrichmentEngine(request_delay=1.0, timeout=10)
    results = enricher.enrich_state('WV', progress_callback=progress_callback)
    ...
    print(f'\n✓ West Virginia enrichment complete!')
