"""
Puerto Rico Territory Enrichment Script

Discovers meeting minutes sources for all Puerto Rico municipalities using
domain verification and URL pattern probing.

Run:
    DATABASE_URL=... python3 scripts/enrich_puertorico.py
"""
import os
import sys
...
def main():
    print('='*60)
    print('PUERTO RICO ENRICHMENT')
    print('='*60)

    enricher = EnrichmentEngine(request_delay=1.0, timeout=10)
    results = enricher.enrich_state('PR', progress_callback=progress_callback)
    ...
    print(f'\n✓ Puerto Rico enrichment complete!')
