"""
Delaware State Enrichment Script

Discovers meeting minutes sources for all Delaware municipalities using
domain verification and URL pattern probing.

Run:
    DATABASE_URL=... python3 scripts/enrich_delaware.py
"""
import os
import sys
...
def main():
    print('='*60)
    print('DELAWARE ENRICHMENT')
    print('='*60)

    enricher = EnrichmentEngine(request_delay=1.0, timeout=10)
    results = enricher.enrich_state('DE', progress_callback=progress_callback)
