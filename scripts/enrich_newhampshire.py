"""
New Hampshire State Enrichment Script

Discovers meeting minutes sources for all New Hampshire municipalities using
domain verification and URL pattern probing.

Run:
    DATABASE_URL=... python3 scripts/enrich_newhampshire.py
"""
import os
import sys
...
def main():
    print('='*60)
    print('NEW HAMPSHIRE ENRICHMENT')
    print('='*60)

    enricher = EnrichmentEngine(request_delay=1.0, timeout=10)
    results = enricher.enrich_state('NH', progress_callback=progress_callback)
    ...
    print(f'\n✓ New Hampshire enrichment complete!')
