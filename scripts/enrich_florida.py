"""
Florida State Enrichment Script

Discovers meeting minutes sources for all Florida municipalities using
domain verification and URL pattern probing.

Run:
    DATABASE_URL=... python3 scripts/enrich_florida.py
"""
import os
import sys
...
def main():
    print('='*60)
    print('FLORIDA ENRICHMENT')
    print('='*60)

    enricher = EnrichmentEngine(request_delay=1.0, timeout=10)
    results = enricher.enrich_state('FL', progress_callback=progress_callback)
