"""
California State Enrichment Script

Discovers meeting minutes sources for all California municipalities using
domain verification and URL pattern probing.

Run:
    DATABASE_URL=... python3 scripts/enrich_california.py
"""
import os
import sys
...
def main():
    print('='*60)
    print('CALIFORNIA ENRICHMENT')
    print('='*60)

    enricher = EnrichmentEngine(request_delay=1.0, timeout=10)
    results = enricher.enrich_state('CA', progress_callback=progress_callback)
    ...
    print(f'\n✓ California enrichment complete!')
